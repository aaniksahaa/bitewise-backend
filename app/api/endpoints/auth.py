...
async def register(user_data: UserRegister, db: AsyncSession = Depends(get_db)) -> Any:
    ...
    # Create and send OTP
    otp, _ = await AuthService.create_otp(
...
async def verify_email(verification_data: EmailVerify, db: AsyncSession = Depends(get_db)) -> Any:
    """
...
async def verify_login(verification_data: LoginVerify, db: AsyncSession = Depends(get_db)) -> Any:
    """
...
async def _process_google_user(google_user: Any, db: AsyncSession) -> GoogleLoginResponse:
    ...
    user = await AuthService.get_user_by_oauth(db, "google", google_user.id)
    is_new_user = False
    first_login = False

    if not user:
        # Check if a user already exists with this email but different provider
        existing_user = await AuthService.get_user_by_email(db, google_user.email)
        ...
        if existing_user:
            ...
            username = await AuthService.generate_unique_username(db, base_username)
...
async def refresh_token(
    refresh_token_data: RefreshTokenRequest, db: AsyncSession = Depends(get_db)
) -> Any:
    """
    Obtain a new access token using a refresh token.
    """
    result = await AuthService.refresh_access_token(db, refresh_token_data.refresh_token)
    if not result:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def logout(
    ...
    """
    await AuthService.revoke_all_refresh_tokens(db, current_user.id)
...
async def request_password_reset(
    ...
    request_id, expires_at = await AuthService.create_password_reset_request(
...
async def complete_password_reset(
    reset_data: PasswordResetComplete, db: AsyncSession = Depends(get_db)
) -> Any:
    """
    Complete the password reset process with the OTP and new password.
    """
    user = await AuthService.verify_password_reset_request(
...
async def debug_login_status(
    ...
    return {
        ...
        "time_since_last_login": (datetime.now(timezone.utc) - user.last_login_at) if user.last_login_at else None,
    } 